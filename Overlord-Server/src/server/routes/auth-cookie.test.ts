...
const ORIGINAL_ENV = {
  OVERLORD_AUTH_COOKIE_SECURE: process.env.OVERLORD_AUTH_COOKIE_SECURE,
  OVERLORD_TLS_OFFLOAD: process.env.OVERLORD_TLS_OFFLOAD,
  OVERLORD_TRUST_PROXY_HEADERS: process.env.OVERLORD_TRUST_PROXY_HEADERS,
};
...
describe("shouldUseSecureAuthCookie", () => {
  test("does not trust X-Forwarded-Proto by default", () => {
    delete process.env.OVERLORD_AUTH_COOKIE_SECURE;
    delete process.env.OVERLORD_TLS_OFFLOAD;
    delete process.env.OVERLORD_TRUST_PROXY_HEADERS;
  ...
  test("treats TLS offload as externally secure without forwarded headers", () => {
    process.env.OVERLORD_TLS_OFFLOAD = "true";
    delete process.env.OVERLORD_TRUST_PROXY_HEADERS;
    delete process.env.OVERLORD_AUTH_COOKIE_SECURE;
  ...
  test("trusts X-Forwarded-Proto when proxy headers are explicitly trusted", () => {
    process.env.OVERLORD_TRUST_PROXY_HEADERS = "true";
    delete process.env.OVERLORD_TLS_OFFLOAD;
    delete process.env.OVERLORD_AUTH_COOKIE_SECURE;
  ...
  test("honors explicit secure override", () => {
    process.env.OVERLORD_AUTH_COOKIE_SECURE = "true";
    const req = new Request("http://localhost/api/login");
    expect(shouldUseSecureAuthCookie(req)).toBe(true);
  });
});
