...
beforeAll(async () => {
  process.env.DATA_DIR = await mkdtemp(join(tmpdir(), "overlord-build-signing-test-"));
...
describe("build-signing", () => {
  ...
  test("token signed with a different key fails verification", async () => {
    ...
    process.env.DATA_DIR = await mkdtemp(join(tmpdir(), "overlord-build-signing-test-rotated-"));
    resetCache();
